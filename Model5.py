...
# usage: python MNISTModel5.py - train the model
...
def Model5(input_tensor=None, train=False, re_train=False, x_train_more=[], y_train_more=[], retrain_num=0):
    nb_classes = 10
    ...
    x = Dense(100, activation='relu', name='fc5')(x)
    x = Dense(100, activation='relu', name='fc6')(x)
    x = Dense(100, activation='relu', name='fc7')(x)
    x = Dense(100, activation='relu', name='fc8')(x)
    x = Dense(132, activation='relu', name='fc9')(x)
    ...
    if train:
        optim = optimizers.adadelta(lr=0.1)
        model.compile(loss='categorical_crossentropy', optimizer='adadelta', metrics=['accuracy'])

        if re_train:
            model.load_weights('./Model5_' + str(retrain_num) + '.h5')
        ...
        if re_train:
            model.save_weights('./Model5_' + str(retrain_num + 1) + '.h5')
        else:
        ...
        model.load_weights('./Model5' + '_' + str(retrain_num) + '.h5')
        print(bcolors.OKBLUE + 'Model5' + '_' + str(retrain_num) + ' loaded' + bcolors.ENDC)
