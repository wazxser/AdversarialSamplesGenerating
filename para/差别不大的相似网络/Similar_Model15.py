...
# usage: python MNISTSimilar_Model15.py - train the model
...
def Similar_Model15(input_tensor=None, train=False, re_train=False, x_train_more=[], y_train_more=[]):
    nb_classes = 10
    ...
    if train:
        ...
        if re_train:
            model.load_weights('./Similar_Model15.h5')
        # trainig
        model.fit(x_train, y_train, validation_data=(x_test, y_test), batch_size=batch_size, epochs=nb_epoch, verbose=1)
        # save model
        if re_train:
            model.save_weights('./Similar_Model15_retrain_' + str(nb_epoch) + '_robustness.h5')
        else:
            model.save_weights('./Similar_Model15.h5')
        ...
        # model.load_weights('./Similar_Model15_retrain_' + str(nb_epoch) + '_robustness.h5')
        model.load_weights('./Similar_Model15.h5')
        print(bcolors.OKBLUE + 'Similar_Model15 loaded' + bcolors.ENDC)

    return model


if __name__ == '__main__':
    Similar_Model15(train=True)
