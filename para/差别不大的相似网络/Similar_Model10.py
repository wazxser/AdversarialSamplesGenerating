...
# usage: python MNISTSimilar_Model10.py - train the model
...
def Similar_Model10(input_tensor=None, train=False, re_train=False, x_train_more=[], y_train_more=[]):
    nb_classes = 10
    ...
    x = Dense(66, activation='relu', name='fc2')(x)
    ...
    if train:
        ...
        if re_train:
            model.load_weights('./Similar_Model10.h5')
        # trainig
        model.fit(x_train, y_train, validation_data=(x_test, y_test), batch_size=batch_size, epochs=nb_epoch, verbose=1)
        # save model
        if re_train:
            model.save_weights('./Similar_Model10_retrain_' + str(nb_epoch) + '_robustness.h5')
        else:
            model.save_weights('./Similar_Model10.h5')
        ...
        # model.load_weights('./Similar_Model10_retrain_' + str(nb_epoch) + '_robustness.h5')
        model.load_weights('./Similar_Model10.h5')
        print(bcolors.OKBLUE + 'Similar_Model10 loaded' + bcolors.ENDC)

    return model


if __name__ == '__main__':
    Similar_Model10(train=True)
