...
def Model2(input_tensor=None, train=False, re_train=False, x_train_more=[], y_train_more=[], retrain_num=0):
    nb_classes = 10
    kernel_size = (5, 5)
    nb_epoch = 1
    ...
    if train:
        # optim = optimizers.adadelta(lr=1)
        model.compile(loss='categorical_crossentropy', optimizer='adadelta', metrics=['accuracy'])
        if re_train:
            model.load_weights('./Model2' + '_' + str(retrain_num) + '.h5')

        model.fit(x_train, y_train, validation_data=(x_test, y_test), batch_size=batch_size, epochs=nb_epoch, verbose=1)

        if re_train:
            model.save_weights('./Model2' + '_' + str(retrain_num+1) + '.h5')
        else:
            model.save_weights('./Model2_0.h5')
        ...
        model.load_weights('./Model2' + '_' + str(retrain_num) + '.h5')
        print(bcolors.OKBLUE + 'Model2' + '_' + str(retrain_num) + ' loaded' + bcolors.ENDC)
    return model


if __name__ == '__main__':
    Model2(train=True, re_train=False)
